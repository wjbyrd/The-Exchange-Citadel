// EXCHANGE CITADEL QUESTION BANK — SHARED-ENGINE REPAIR BUILD 20260717_1
...
function ecDisplayedChoiceKey(value){
  const formatted = ecFormatOption(value).trim().replace(/\s+/g, " ");
  const numericMatch = formatted.match(/^([+-]?)(\$?)(\d[\d,]*(?:\.\d+)?)(%?)$/);

  if(numericMatch){
    const numericValue = Number(`${numericMatch[1] || ""}${numericMatch[3].replace(/,/g, "")}`);
    if(Number.isFinite(numericValue)){
      return `numeric:${numericMatch[2] || ""}:${numericValue}:${numericMatch[4] || ""}`;
    }
  }

  return `text:${formatted}`;
}
function ecUniqueOptions(correct, wrongs){
  const options = [];
  const seen = new Set();
  const add = value => {
    const formatted = ecFormatOption(value);
    const key = ecDisplayedChoiceKey(formatted);
    if(!seen.has(key)){
      seen.add(key);
  ...
  if(Number.isFinite(numericCorrect)){
    const baseStep = Math.max(1, Math.abs(numericCorrect) >= 10 ? Math.round(Math.abs(numericCorrect) * 0.1) : 0.25);
    [1,-1,2,-2,3,-3,4,-4].forEach(multiplier => add(numericCorrect + (baseStep * multiplier)));
  } else {
    ["none of these", "not enough information", "the opposite result", "no change"].forEach(add);
  }

  return options.slice(0, 4);
}
function makeECGeneratedQuestion(id, q, correct, wrongs, tag, type, objective, skill, feedback, hint){
  return {id,q,options:ecUniqueOptions(correct, wrongs),a:0,tag,type,objective,difficulty:"generated",primarySkill:skill,repairSkill:skill,conceptCluster:"generated_calculation",feedback,hint};
}
...
function buildDirectSkillRoutes(sourcePools){
  const entries = Object.entries(sourcePools || {}).filter(([, pool]) => Array.isArray(pool) && pool.length);
  const routes = {};

  Object.values(window.questionBanks || {}).flat().forEach(question => {
    if(!question?.primarySkill) return;

    const match = entries.find(([, pool]) => pool.some(item =>
      item.objective === question.objective && item.tag === question.tag
    )) || entries.find(([, pool]) => pool.some(item =>
      item.objective === question.objective
    )) || entries.find(([, pool]) => pool.some(item =>
      item.tag === question.tag
    ));

    if(!match) return;
    const route = routes[question.primarySkill] || [];
    match[1].forEach(item => {
      if(!route.some(existing => existing.id === item.id)) route.push(item);
    });
    routes[question.primarySkill] = route;
  });

  return routes;
}

window.directSkillRepairRoutes = buildDirectSkillRoutes(window.microSkillRepairPools);
window.directSkillBridgeRoutes = buildDirectSkillRoutes(window.microSkillBridgePools);
